...
const sinceArray = () =>
  Array.from({ length: 21 }).map((_, i) =>
    format(addDays(new Date(), 0 - i), 'yyyy-MM-dd')
  );
const sinceYesterday = () =>
  `${format(addDays(new Date(), -1), 'yyyy-MM-dd')}T00:00:00.000Z`;
const sinceLastWeek = () =>
  `${format(addDays(new Date(), -8), 'yyyy-MM-dd')}T00:00:00.000Z`;
...
export class BitQueryService {
  ...
  constructor(
    ...
    this.token =
      '+LDiubcQUptEmjNSJQICeNWkdi77X3iAKQQ8dSyD+VkhwPhDf3lOj6oZyqnAmube1UyI57igIekZb87YHEMYjQ==';
  }
      ...
      try {
        try {
          const cache = JSON.parse(
            (await this.redisClient.get(`${cacheKey}:${offsetStep + 1}`)) ||
              'null'
          );
          if (cache) {
            return;
        ...
        const {
          data: {
            stats: { tradersDistributionValue },
          },
        } = await got.post<BitQueryStatsTradersDistributionValueQuery>(
          'https://explorer.bitquery.io/proxy_graphql',
          {
            json: {
              query: `
        ...
        await this.redisClient.set(
          cacheKey,
          JSON.stringify(offsetStep + 1),
          'PX',
          72 * 60 * 60 * 1000
        );

        const minAmount: number =
          Math.min(...tradersDistributionValue.map((t) => t.tradeAmount)) || 0;
        const maxAmount: number =
          Math.max(...tradersDistributionValue.map((t) => t.tradeAmount)) || 0;

        try {
          await this.redisClient.set(
            `${cacheKey}:${offsetStep + 1}`,
            JSON.stringify(tradersDistributionValue),
            'PX',
            72 * 60 * 60 * 1000
          );
        } catch (e) {
        ...
        try {
          const prevMin = JSON.parse(
            (await this.redisClient.get(`${cacheKey}:min`)) || 'null'
          );

          await this.redisClient.set(
            `${cacheKey}:min`,
            JSON.stringify(
              !prevMin || minAmount < prevMin ? minAmount : prevMin
            ),
            'PX',
            72 * 60 * 60 * 1000
          );
        ...
        try {
          const prevMax = JSON.parse(
            (await this.redisClient.get(`${cacheKey}:max`)) || 'null'
          );

          await this.redisClient.set(
            `${cacheKey}:max`,
            JSON.stringify(
              !prevMax || maxAmount > prevMax ? maxAmount : prevMax
            ),
            'PX',
            72 * 60 * 60 * 1000
          );
        ...
        if (
          !tradersDistributionValue.length ||
          tradersDistributionValue.length < 25000
        ) {
          break;
        }
      } catch (e) {
  ...
  async statsTransfers(
    ...
    try {
      ...
      Object.values(data)
        ...
        .forEach(
          (
            item: BitQueryStatsTransfersQuery['data']['stats']['transfers'][0]
          ) => {
            if (
              ...
              set(
                map,
                `${item.date.date}.medianTransferAmount`,
                item.median_transfer_amount
              );
              set(
                map,
                `${item.date.date}.averageTransferAmount`,
                item.average_transfer_amount
              );
              set(
                map,
                `${item.date.date}.medianTransferAmountUsd`,
                item.median_transfer_amount_usd
              );
              set(
                map,
                `${item.date.date}.averageTransferAmountUsd`,
                item.average_transfer_amount_usd
              );
            }
            set(
              map,
              `${item.date.date}.totalAmount`,
              item.total_amount + get(map, `${item.date.date}.totalAmount`, 0)
            );
            set(
              map,
              `${item.date.date}.totalAmountUsd`,
              item.total_amount_usd +
                get(map, `${item.date.date}.totalAmountUsd`, 0)
            );
            set(
              map,
              `${item.date.date}.uniqReceivers`,
              item.uniq_receivers +
                get(map, `${item.date.date}.uniqReceivers`, 0)
            );
            set(
              map,
              `${item.date.date}.uniqSenders`,
              item.uniq_senders + get(map, `${item.date.date}.uniqSenders`, 0)
            );
            set(
              map,
              `${item.date.date}.transferCount`,
              item.transfer_count +
                get(map, `${item.date.date}.transferCount`, 0)
            );
          }
        );
      const result = Object.entries(map).map(([date, rest]: [string, any]) => ({
        date,
        ...rest,
      }));
      await this.redisClient.set(
  ...
  async statsTransfersNew(
    values: { address: string; network: string }[],
    variables: { since: string; till: string }
  ): Promise<
    {
      date: string;
  ...
  > {
    const data: (Omit<
      BitQueryStatsTransfersNewQuery['data']['stats']['transfers'][0],
      'date'
    > & { date: string })[] = (
      await Promise.all(
        values.map(async ({ address, network }) => {
          const today = format(new Date(), 'yyyy-MM-dd');
          return (
            await this.getStatsTransfersNew({
              network,
              token: address,
              ...variables,
            })
    ...
    return Object.entries(
      data.reduce<{
        ...
        if (
          item.totalAmount > Number(get(prev, `${item.date}.totalAmount`, 0))
        ) {
          set(
            prev,
            `${item.date}.medianTransferAmount`,
            item.medianTransferAmount
          );
          set(
            prev,
            `${item.date}.averageTransferAmount`,
            item.averageTransferAmount
          );
          set(
            prev,
            `${item.date}.medianTransferAmountUsd`,
            item.medianTransferAmountUsd
          );
          set(
            prev,
            `${item.date}.averageTransferAmountUsd`,
            item.averageTransferAmountUsd
          );
        }
        set(
          prev,
          `${item.date}.totalAmount`,
          item.totalAmount +
            get<any, string>(prev, `${item.date}.totalAmount`, 0)
        );
        set(
          prev,
          `${item.date}.totalAmountUsd`,
          item.totalAmountUsd +
            get<any, string>(prev, `${item.date}.totalAmountUsd`, 0)
        );
        set(
          prev,
          `${item.date}.uniqReceivers`,
          item.uniqReceivers +
            get<any, string>(prev, `${item.date}.uniqReceivers`, 0)
        );
        set(
          prev,
          `${item.date}.uniqSenders`,
          item.uniqSenders +
            get<any, string>(prev, `${item.date}.uniqSenders`, 0)
        );
        set(
          prev,
          `${item.date}.transferCount`,
          item.transferCount +
            get<any, string>(prev, `${item.date}.transferCount`, 0)
        );
        return prev;
      }, {})
  ...
  > {
    const data: (Omit<
      BitQueryStatsSwapsQuery['data']['stats']['swaps'][0],
      'date'
    > & { date: string })[] = (
      await Promise.all(
        values.map(async ({ address, network }) => {
    ...
    return Object.entries(
      data.reduce<{
        ...
        set(
          prev,
          `${item.date}.tradeAmountUsd`,
          item.tradeAmountUsd +
            get<any, string>(prev, `${item.date}.tradeAmountUsd`, 0)
        );
        set(
          prev,
          `${item.date}.countTxs`,
  ...
  async statsHoldersNew(
    values: { address: string; network: string }[],
    date: string
  ): Promise<number> {
    const data: number[] = await Promise.all(
      values.map(async ({ address, network }) =>
        this.getStatsHoldersNew({
          network,
          token: address,
          till: date,
        })
  ...
  async statsSwaps(
    ...
    try {
      ...
      Object.values(data)
        .reduce((p, n) => [...p, ...n], [])
        .forEach(
          (item: BitQueryStatsSwapsQuery['data']['stats']['swaps'][0]) => {
            set(
              map,
              `${item.date.date}.tradeAmountUsd`,
              item.tradeAmountUsd +
                get(map, `${item.date.date}.tradeAmountUsd`, 0)
            );
            set(
              map,
              `${item.date.date}.countTxs`,
  ...
  async statsHolders(
    ...
    try {
              ...
              switch (key) {
                case 'btcAddress': {
                  return [
                    'btc',
                    token
                      ? await promiseMap(sinceArray(), async (till) => ({
                          date: till,
                          count: get(
                            await this.getStatsHolders({
                              network: 'bsc',
                              token,
                              till,
                            }),
      ...
      Object.values(data)
        .reduce((p, n) => [...(p || []), ...(n || [])], [])
        .forEach((item: { date: string; count: number }) => {
          set(
            map,
            `${item.date}.count`,
  ...
  async statsTradersDistributionValue(
    btcAddress?: string,
    ethAddress?: string,
    update = false
  ): Promise<any> {
    const sinceYesterdayDate = sinceYesterday();
    const till = tillToday();
    const cacheKey = `cmc:tradersDistributionValue:${md5(
      `${btcAddress}_${ethAddress}`
    )}:${sinceYesterdayDate}:${till}`;
    try {
      const cache = JSON.parse(
        (await this.redisClient.get(cacheKey)) || 'null'
      );
      ...
      const trades = (
            ...
            switch (key) {
              case 'btcAddress': {
                return [
                  'btc',
                  token
                    ? await this.getStatsTradersDistributionValue({
                        network: 'bsc',
                        token,
                        since: sinceYesterdayDate,
                        till,
                      })
                    : undefined,
                ];
        ...
        BitQueryStatsTradersDistributionValueQuery['data']['stats']['tradersDistributionValue']
      >(
        (prev, [, data]) =>
          Array.isArray(data) && data.length ? [...prev, ...data] : prev,
        []
      );
      ...
      trades.reduce((userStepsCount: string[], trade) => {
        const stepIndex = steps.findIndex(({ tradeAmount }, index) => {
          ...
          return (
            ...
            (lastStep ||
              trade.tradeAmount < get(steps, `${index + 1}.tradeAmount`, 0))
          );
        });
        ...
        [...new Set([trade.maker.address, trade.taker.address])].forEach(
          (user) => {
      ...
      const result = steps
        ...
        .reduce((prev, { tradeAmount, userCount, swapsCount }) => {
          ...
          if (prevUserCount >= userCount || prevSwapsCount >= swapsCount) {
            return [
              ...prev.filter((_, i) => i !== prev.length - 1),
              {
  ...
  async statsPairStatistics(
    ...
    const [btcAddress, btcPoolContract] = btcAddress_poolContract.reduce<
      [string, string[]]
    >(
      (_prev, pair) => {
        const [, prev] = _prev;
    ...
    const [ethAddress, ethPoolContract] = ethAddress_poolContract.reduce<
      [string, string[]]
    >(
      (_prev, pair) => {
        const [, prev] = _prev;
    ...
    const sinceLastWeekDate = sinceLastWeek();
    const till = tillToday();

    const cacheKey = `cmc:pairStatistics:${md5(
      `${btcPoolContract.join('_')}_${ethPoolContract.join('_')}`
    )}:${sinceLastWeekDate}:${till}`;
    ...
    try {
      ...
      const trades = (
            ...
            switch (key) {
              case 'btcContract': {
                return [
                  'btc',
                  contracts.length
                    ? await this.getStatsPairStatistics({
                        network: 'bsc',
                        contracts,
                        since: sinceLastWeekDate,
                        till,
                      })
                    : undefined,
                ];
        ...
        BitQueryStatsPairStatisticsQuery['data']['stats']['pairStatistics']
      >(
        (prev, [, data]) =>
          Array.isArray(data) && data.length ? [...prev, ...data] : prev,
        []
      ...
      const result = (({
        ...
        return {
          ...
          buyersAndSellersCount: [
            ...new Set([...buyersUnique, ...sellersUnique]),
          ].length,
        };
      })(
        trades.reduce(
          (prev, trade) => {
            ...
            if (tokenAddresses.includes(trade.buyCurrency.address)) {
              temp.tradesBuyCount++;
              temp.buyers.push(trade.taker.address);
              temp.buyersVolume += trade.buyAmount;
            } else {
              temp.tradesSellCount++;
              temp.sellers.push(trade.taker.address);
              temp.sellersVolume += trade.sellAmount;
            }
  ...
  async getBalancesOfPair(variables: {
    ...
    try {
      const response = await got.post<BitQueryBalanceOfPairQuery>(
        'https://explorer.bitquery.io/proxy_graphql',
        {
          json: {
            query: `
      ...
      return Object.fromEntries(
        (get(response, 'data.pair.liquidity.0.balances', []) || []).map((_) => [
  ...
  private async getStatsTransfers(variables: {
    ...
    } = await got.post<BitQueryStatsTransfersQuery>(
      'https://explorer.bitquery.io/proxy_graphql',
      {
        json: {
          query: `
  ...
  private async getStatsTransfersNew(variables: {
    ...
    } = await got.post<BitQueryStatsTransfersNewQuery>(
      'https://explorer.bitquery.io/proxy_graphql',
      {
        json: {
          query: `
  ...
  private async getStatsSwapsNew(variables: {
    ...
    } = await got.post<BitQueryStatsSwapsQuery>(
      'https://explorer.bitquery.io/proxy_graphql',
      {
        json: {
          query: `
  ...
  private async getStatsHoldersNew(variables: {
    ...
    } = await got.post<BitQueryStatsHoldersQuery>(
      'https://explorer.bitquery.io/proxy_graphql',
      {
        json: {
          query: `
  ...
  > {
    ...
    while (offsetStep < DEEP_OF_STEP) {
      try {
        ...
        } = await got.post<BitQueryStatsPairStatisticsQuery>(
          'https://explorer.bitquery.io/proxy_graphql',
          {
            json: {
              query: `
